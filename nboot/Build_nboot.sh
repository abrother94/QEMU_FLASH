...
if [ "$1" == "C" ];then
    rm *.o ; rm nboot.map ; rm nboot.elf.srec ;rm nboot.bin ;rm nboot.elf; rm ../nboot.bin ; rm ../nboot.elf
else
...
arm-linux-gnueabi-gcc  -g  -Os  -mabi=aapcs-linux -c -o start.o start.S

arm-linux-gnueabi-gcc -g  -Os   -mabi=aapcs-linux -c -o hello.o hello.c

arm-linux-gnueabi-ld -Bstatic -T nboot.lds $UNDEF_SYM  -Ttext 0x01000000 hello.o  start.o -o nboot.elf -Map nboot.map
